...
export function useUnifiedErrorHandler(options: UseUnifiedErrorHandlerOptions = {}) {
    ...
    const showError = useCallback((type: ErrorType, message: string, details?: string) => {
    ...
    const handleApiError = useCallback((err: unknown, context?: string) => {
        ...
        // Validation errors
            ...
            showError(
                "validation",
                err.response?.data?.detail || "Invalid parameters provided. Please check your input values."
            );
        }
        ...
        // Unknown errors
            ...
            showError(
                "unknown",
                err.response?.data?.detail || err.message || "An unexpected error occurred. Please try again."
            );
        }
    }, [showError]);
    ...
    const handleAuthError = useCallback((err: unknown) => {
        ...
        if (!isError(err)) {
            showError("unknown", "An unexpected authentication error occurred. Please try again.");
            return;
        }
        ...
        if (err.response?.status === 401) {
            ...
            showError(
                "auth",
                err.response?.data?.detail || "Account is not active. Please check your email for verification."
            );
