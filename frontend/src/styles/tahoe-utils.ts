 ...
 */
export function getStyleCombination(combination: keyof typeof styleCombinations): string {
 ...
 */
export function getSliderProgressStyles(progress: number, isAngle: boolean = false, min: number = 0, max: number = 100) {
    const baseStyles = {
    ...
    if (isAngle) {
        ...
        return {
            ...
            width: progress < 0 ? `${Math.abs(progress / min) * 50}%` : `${(progress / max) * 50}%`,
        };
    }
 ...
 */
export function getResponsiveStyles(baseStyles: string, responsiveOverrides?: {
    sm?: string;
    md?: string;
    lg?: string;
    xl?: string;
