...
export interface TendonAnalysisResponse {
    result: {
        ...
        actuation_commands: Record<string, {
...
export const robotAPI = {
    ...
    computePCCWithTendons: async (params: PCCParams, retryConfig?: Partial<RetryConfig>): Promise<TendonAnalysisResponse> => {
        return withRetry(
            async () => {
                // Use Tauri client for desktop app, fallback to axios for web
                if (typeof window !== 'undefined' && (window as any).__TAURI__) {
                    const response = await tauriClient.post<TendonAnalysisResponse>('/pcc-with-tendons', params);
                    if (!response.success) {
                        throw new Error(response.error || 'API call failed');
                    }
